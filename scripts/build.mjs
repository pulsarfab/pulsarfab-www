...
const pages = ['index.html', '404.html', ...docs.map(file => `docs/${file}`)];
const stale = [];
const escape = text => text.replaceAll('&', '&amp;').replaceAll('"', '&quot;').replaceAll('<', '&lt;').replaceAll('>', '&gt;');
const url = file => `${siteUrl}/${file === 'index.html' ? '' : file === 'docs/index.html' ? 'docs/' : file}`;
...
<header class="site-header">
  <nav class="nav" aria-label="Primary navigation">
    ...
    <div class="nav-links">
      <a class="keep" href="${prefix}docs/"${file === 'docs/index.html' ? ' aria-current="page"' : ''}>regain</a>
      <a class="keep" href="${prefix}#projects">Hardware</a>
      <a href="https://github.com/pulsarfab">GitHub</a>
      <a class="cta" href="${prefix}docs/install.html"${file === 'docs/install.html' ? ' aria-current="page"' : ''}>Install</a>
    </div>
  </nav>
</header>`;
}
...
for (const file of pages) {
  const prefix = file === '404.html' ? '/' : file.startsWith('docs/') ? '../' : '';
  let html = fs.readFileSync(path.join(root, file), 'utf8');
  html = replace(html, 'site-header', header(file, prefix), file);
  html = replace(html, 'release-note', `<div class="release-note">${release.published
    ? `regain ${release.version} · <a href="${sourceUrl}/releases/tag/v${release.version}">Read the release notes</a>`
    : `regain ${release.version} documentation · Signed release pending. <a href="${prefix}docs/install.html#source">Build the tagged source</a> or view the <a href="${sourceUrl}/releases">current public release, ZWOgain ${release.previous}</a>.`}</div>`, file);
  ...
  <a href="${prefix}docs/">regain docs</a>
  ...
  <meta property="og:image" content="${siteUrl}/assets/regain.png">
